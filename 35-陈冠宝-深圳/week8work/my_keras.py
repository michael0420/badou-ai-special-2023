...
network.add(layers.Dense(512, activation='relu', input_shape=(28 * 28,)))  # 全连接层，激活函数为relu
...
train_images = train_images.astype('float32') / 255  # 归一化，可提高速度增加模型泛化能力

test_images = test_images.reshape((10000, 28 * 28))
test_images = test_images.astype('float32') / 255  # 归一化，可提高速度增加模型泛化能力
...
epochs:每次计算的循环是五次
'''
network.fit(train_images, train_labels, epochs=5, batch_size=128)  # network是在第47行开始构建的一个神经网络#训练
...
识别效果与硬件有关（CPU/GPU）.
'''
test_loss, test_acc = network.evaluate(test_images, test_labels, verbose=1)  # 测试
print(test_loss)
...
plt.show()
test_images = test_images.reshape((10000, 28 * 28))
res = network.predict(test_images)  # 推理
